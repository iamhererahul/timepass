...
if (isset($_FILES['profile_picture']) && $_FILES['profile_picture']['error'] == 0) {
    ...
    if (in_array($fileExtension, $allowedExtensions) && $fileSize <= $maxFileSize) {
        ...
        $sql = "SELECT file_path FROM seeker_profile_pictures WHERE user_id = :user_id";
        ...
        if ($existingFile) {
            ...
            $sql = "DELETE FROM seeker_profile_pictures WHERE user_id = :user_id";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([':user_id' => $userId]);
        }
        ...
        if (move_uploaded_file($fileTmpPath, $dest_path)) {
            ...
            $sql = "INSERT INTO seeker_profile_pictures (user_id, file_name, file_path) VALUES (:user_id, :file_name, :file_path)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                ':user_id' => $userId,
